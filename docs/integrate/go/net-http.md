...
creating misleading deltas or apparent restarts. Multi-process and replica
...
Counters, duration, uptime, and start identity belong to one process. StatLite
does not aggregate processes or replicas. Those deployments are outside this
helper's supported model because cumulative counters can decrease and process
identity can alternate. If you need one of those setups,
...
fields. Do not perform blocking dependency checks during a metrics request.
